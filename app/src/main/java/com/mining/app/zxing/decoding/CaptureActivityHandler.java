...
import com.musicbase.ui.activity.MipcaActivityCapture;
...
//	public CaptureActivityHandler(AutoFocusActivity activity, Vector<BarcodeFormat> decodeFormats, String characterSet) {
//		this.activity = activity;
//		isAuto = true;
//		decodeThread = new DecodeThread(activity, decodeFormats, characterSet, new ViewfinderResultPointCallback(activity.getViewfinderView()));
//		decodeThread.start();
...
//			if (isAuto)
//				((AutoFocusActivity) activity).handleDecode((Result) message.obj, barcode);// ���ؽ��?
//			else
				((MipcaActivityCapture) activity).handleDecode((Result) message.obj, barcode);
			...
			state = State.PREVIEW;
			CameraManager.get().requestPreviewFrame(decodeThread.getHandler(), R.id.decode,false);
	...
	private void restartPreviewAndDecode() {
		if (state == State.SUCCESS) {
			...
			CameraManager.get().requestAutoFocus(this, R.id.auto_focus);
//			if (isAuto)
//				((AutoFocusActivity) activity).drawViewfinder();
//			else
				((MipcaActivityCapture) activity).drawViewfinder();
		}
	}
